...
import com.chenh.catalina.pms.dao.ProductAttributeValueDao;
import com.chenh.catalina.pms.dao.ProductDao;
...
import com.chenh.common.mybatis.mapper.*;
...
public class SpuServiceImpl implements SpuService {
    ...
    @Override
    public List<PmsProduct> list(ProductQueryParam productQueryParam, Integer pageSize, Integer pageNum) {
...
//        record.setNewStatus(newStatus);
    ...
    public List<ProductCategoryNode> categoryTreeList() {
        ...
        List<ProductCategoryNode> result = categoryList.stream()
                .filter(item -> item.getParentId().equals(0L))
                .map(item -> convert(item, categoryList))
                .collect(Collectors.toList());
        return result;
    }

    /**
     * 初始对象转化为节点对象,递归的设置好各级分类
     * @param item
     * @param categoryList
     * @return
     */
    private ProductCategoryNode convert(PmsProductCategory item, List<PmsProductCategory> categoryList) {
        ProductCategoryNode node = new ProductCategoryNode();
        BeanUtils.copyProperties(item, node);
        List<ProductCategoryNode> childNodes = categoryList.stream()
                .filter(subItem -> subItem.getParentId().equals(item.getId()))
                .map(subItem -> convert(subItem, categoryList))
                .collect(Collectors.toList());
